...
const services = [
  ...
  {
    icon: "🛁", title: "Bathroom Renovation",
  ...
  {
    icon: "🏠", title: "Full Home Renovation",
  ...
  {
    icon: "🪜", title: "Loft & Attic Conversion",
  ...
  {
    icon: "💻", title: "Home Office Renovation",
...
export default function ServicesPage() {
  return (
    <>
      {/* Hero */}
      <section style={{ paddingTop: "140px", paddingBottom: "5rem", paddingLeft: "1.5rem", paddingRight: "1.5rem", background: "linear-gradient(180deg,var(--dark) 0%,#2d2521 100%)", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: "700px", margin: "0 auto" }}>
          <span className="section-label" style={{ color: "var(--accent)" }}>Our Services</span>
          <div className="divider-center" />
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(2.5rem,5vw,4rem)", fontWeight: 700, color: "white", margin: "0 0 1.25rem" }}>Every renovation service your home needs</h1>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "1.1rem", lineHeight: 1.75, marginBottom: "2rem" }}>From a single-room refresh to a complete home overhaul — we bring expertise, transparency, and quality to every project.</p>
          <Link href="/contact" className="btn-accent" style={{ fontSize: "1rem" }}>Book Free Consultation <ArrowRight size={16} /></Link>
        </div>
      </section>
      ...
      <section style={{ padding: "6rem 1.5rem", background: "var(--bg)" }}>
        <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "5rem" }}>
          {services.map((s, i) => (
            <div key={s.title} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center", direction: i % 2 === 1 ? "rtl" : "ltr" }} className="service-row">
              <div style={{ direction: "ltr" }}>
                <div style={{ fontSize: "2.5rem", marginBottom: "1rem" }}>{s.icon}</div>
                <h2 style={{ fontFamily: "var(--font-display)", fontSize: "2rem", fontWeight: 700, color: "var(--text)", marginBottom: "1rem" }}>{s.title}</h2>
                <p style={{ color: "var(--text-muted)", lineHeight: 1.75, marginBottom: "1.75rem" }}>{s.desc}</p>
                <ul style={{ listStyle: "none", padding: 0, margin: "0 0 2rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.6rem" }}>
                  {s.features.map((f) => (
                    <li key={f} style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.875rem", color: "var(--text)" }}>
                      <CheckCircle2 size={15} style={{ color: "var(--accent)", flexShrink: 0 }} />{f}
                    </li>
                  ))}
                </ul>
                <Link href="/contact" className="btn-accent">Get a Quote <ArrowRight size={16} /></Link>
              </div>
              <div style={{ direction: "ltr", borderRadius: "20px", overflow: "hidden", height: "380px" }}>
                <img src={s.img} alt={s.title} className="img-zoom" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "5rem 1.5rem", background: "var(--accent-light)", textAlign: "center" }}>
        <div style={{ maxWidth: "560px", margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: "2.25rem", fontWeight: 700, color: "var(--text)", marginBottom: "1rem" }}>Not sure which service you need?</h2>
          <p style={{ color: "var(--text-muted)", marginBottom: "2rem", lineHeight: 1.75 }}>Our renovation consultants will assess your home and recommend the best approach — for free.</p>
          <Link href="/contact" className="btn-accent">Book Free Assessment <ArrowRight size={16} /></Link>
        </div>
      </section>
    </>
  );
